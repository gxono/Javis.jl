...
"""
    _javis_viewer(video::Video, total_frames::Int, object_list::Vector, show::Bool=true)

Internal Javis Viewer built on Gtk that is called for live previewing.

Provided by the `JavisGtkViewerExt` package extension: load `Gtk` and `GtkReactive`
(e.g. `using Gtk, GtkReactive`) before calling `render(...; liveview = true)` outside of
Jupyter/Pluto.
"""
function _javis_viewer(args...; kwargs...)
    error(
        "The live preview window needs Gtk and GtkReactive to be loaded. " *
        "Run `using Gtk, GtkReactive` before calling `render(...; liveview = true)`.",
    )
end
