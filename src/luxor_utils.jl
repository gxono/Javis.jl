...
function update_currentjpath(action::Symbol)
    ...
    # NOTE: these are unexported/underscore-prefixed Luxor internals (no public
    # equivalent exists as of Luxor 3.8), so they can rename again on a Luxor
    # bump without a deprecation warning. They were called without the leading
    # underscore pre-Luxor 3.x; that name no longer exists.
    r = Luxor._get_current_redvalue()
    g = Luxor._get_current_greenvalue()
    b = Luxor._get_current_bluevalue()
    a = Luxor._get_current_alpha()
